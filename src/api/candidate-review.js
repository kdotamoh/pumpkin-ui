...
export async function getRecruitmentCycleReviewSummary(cycleReference) {
  try {
    ...
    message.error(
      `Cannot get recruitment cycle review summary: ${responseMessage}`
    );
  }
}
...
export async function getApplicationReviewerSummary(reviewerCode) {
  try {
    ...
    message.error(
      `Cannot get application reviewer summary: ${responseMessage}`
    );
  }
}
...
export async function getCandidateApplicationReviews(
  ...
  try {
    ...
    message.error(
      `Cannot get candidate application reviews summary: ${responseMessage}`
    );
  }
}

export async function searchCandidateApplicationReviews(
  reviewerCode,
  seoDecision,
  searchKey
) {
