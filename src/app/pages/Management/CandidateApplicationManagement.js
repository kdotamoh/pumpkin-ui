...
export class CandidateApplicationManagementComponent extends React.Component {
  ...
  onSearchCandidateApplications = () => {
    ...
    this.props.searchCandidateApplications(
      this.props.searchFilters,
      this.props.cycleReference
    );
  };
  ...
  onRecruitmentCycleSelected = (code) => {
    ...
    this.props
      .getCandidates(code, 1)
      .then(() => this.setState({ candidatesHasBeenLoaded: true }));
  };
  ...
  subHeaderView = () => {
    ...
    const cyclesChildren = this.getDropdownChildren(
      this.props.recruitmentCycles
    );
    ...
    return (
      <div className="applicants_page_subheader">
          ...
          <div>
            <div className="flex flex-end margin-bottom-40">
              {(isSuperAdmin || isAdmin) && (
                <Button
                  ...
                  className="semi-bold"
                  onClick={() =>
                    this.setState({ bulkRejectModalVisible: true })
                  }
                  danger
                >
                  Bulk Reject
            ...
            <div className="applicants_page_subheader_row data-row">
              ...
              <div>
                ...
                <Select
                  ...
                  onSelect={(code, dropdownData) =>
                    this.props.onSearchFilterSelected(
                      code,
                      dropdownData,
                      'stageCode',
                      'stage'
              ...
              <div>
                <p className="">Track</p>
                <Select
                  value={this.props.dropdownValues.track}
                  style={dropDownStyle}
                  onSelect={(code, dropdownData) =>
                    this.props.onSearchFilterSelected(
                      code,
            ...
            <div className="applicants_page_subheader_row space-between">
              <div style={{ display: 'flex' }}>
                <div style={{ marginRight: '10px' }}>
                  ...
                  <Select
                    value={this.props.dropdownValues.country}
                    style={dropDownStyle}
                    onSelect={(code, dropdownData) =>
                      this.props.onSearchFilterSelected(
                        code,
                ...
                <div>
                  ...
                  <Select
                    value={this.props.dropdownValues.status}
                    style={{ width: 100 }}
                    onSelect={(code, dropdownData) =>
                      this.props.onSearchFilterSelected(
                        code,
                ...
                <p className="filter_count">
                  Displaying {this.props.displayingCandidates} of{' '}
                  {this.props.totalCandidates} applicants
                </p>
              </div>
              <div
                className="subheader_actions"
                style={{ justifyContent: 'flex-end' }}
              >
                <Button
                  type="primary"
                  onClick={() => this.onSearchCandidateApplications()}
                >
                  Apply Filter
                </Button>
                <Button
                  style={{
                    marginRight: isSuperAdmin || isAdmin ? '5px' : '15px',
                  }}
                  onClick={() =>
                    this.onRecruitmentCycleSelected(this.props.cycleReference)
                  }
                >
                  Clear Filter
                </Button>
        ...
        >
          ...
          />
          <p style={{ color: 'red' }}>
            Clicking reject will automatically reject all the applicant
            currently on the specified stage.
          </p>
        </Modal>
      </div>
    );
...
const mapDispatchToProps = (dispatch) => ({
  getCandidates: (code, currentPage) =>
    dispatch(getCandidates(code, currentPage - 1)),
  getCountriesForSearch: () => dispatch(getCountriesForSearch()),
  searchCandidateApplications: (searchKeys, cycleReference) =>
    dispatch(searchCandidateApplication(searchKeys, cycleReference)),
  setCurrentCandidate: (candidate) => dispatch(setCurrentCandidate(candidate)),
  getRecruitmentCycleDetails: (code) =>
    dispatch(getRecruitmentCycleDetails(code)),
  ...
  handleTextInput: (e, target) =>
    dispatch(onTextInputChanged(target, e.target.value)),
