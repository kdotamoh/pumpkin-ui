...
export class CandidateApplicationReviewsComponent extends React.Component {
  ...
  columns = [
    ...
    },
    {
      ...
      dataIndex: 'decision',
      key: 'decision',
      render: (text, record) => this.decoratedDecisionStatusView(record.decision),
    },
    {
      title: 'SEO\'s  decision',
      dataIndex: 'seoDecision',
      key: 'seoDecision',
      render: (text, record) => this.decoratedDecisionStatusView(record.seoDecision),
    },
    {
      title: 'Actions',
      dataIndex: 'actions',
  ...
  componentDidMount() {
    this.props.getApplicationReviewerSummary(this.props.match.params.reviewerCode);
    this.props.getCandidateApplicationReviews(this.props.match.params.reviewerCode, this.state.seoDecision);
  }
    ...
    return (
      <div>
        ...
        <Modal title="Reviewer's notes" visible={this.state.isNotesModalVisible}
          ...
          <p style={{ padding: 8, border: 'gainsboro 1px solid', borderRadius: 2, marginTop: 32 }}>
            {this.state.selectedCandidate.remarks}
          </p>
        </Modal>

        <Modal title="Application profile" visible={this.state.isDecisionModalVisible}
          okButtonProps={{ style: { display: 'none' } }}
          cancelButtonProps={{ style: { display: 'none' } }}
          onCancel={() => this.setState({ isDecisionModalVisible: false, seoRemarks: '' })}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginRight: 8 }}>
            <div>
              <div>
                <p style={{ margin: 0, fontSize: 12 }}>Applicant</p>
          ...
          <p style={{ padding: 8, border: 'gainsboro 1px solid', borderRadius: 2, marginTop: 24 }}>
            {this.state.selectedCandidate.remarks}
          </p>
          <textarea
            type="text"
            name="remarks"
            style={{ marginTop: 16, marginBottom: 16, borderRadius: 4, width: '100%', padding: 4 }}
            rows="3"
  ...
  getTitle = () => {
    let title = 'REVIEWER';
    if (this.props.currentApplicationReviewer && this.props.currentApplicationReviewer.name) {
  ...
  onSearchCandidateApplicationReviews = async (searchKey) => {
    this.props.searchCandidateApplicationReviews(this.props.match.params.reviewerCode, this.state.seoDecision, searchKey);
  };
  ...
  applicationReviewerSummaryView = () => {
    ...
    return (
      <>
        {showSummary && <div style={{ marginLeft: 'auto' }}>
          <p className="management-component__subheader_title">SEO Decision</p>
          <div style={{ display: 'flex' }}>
            <div style={{ cursor: 'pointer' }} onClick={() => this.onRewiewerSummarySelected()}>
              <SummaryBadgeComponent title="Total" count={total} type="primary" />
            </div>
            <div style={{ cursor: 'pointer' }} onClick={() => this.onRewiewerSummarySelected('APPROVED')}>
              <SummaryBadgeComponent title="Approved" count={approved} type="success"
                isActive={this.state.seoDecision === 'APPROVED'} />
            </div>
            <div style={{ cursor: 'pointer' }} onClick={() => this.onRewiewerSummarySelected('PENDING')}>
              <SummaryBadgeComponent title="Pending" count={pending} type="warning"
                isActive={this.state.seoDecision === 'PENDING'} />
            </div>
            <div style={{ cursor: 'pointer' }} onClick={() => this.onRewiewerSummarySelected('REJECTED')}>
              <SummaryBadgeComponent title="Rejected" count={rejected} type="danger"
  ...
  onRewiewerSummarySelected = (seoDecision) => {
    ...
    this.props.getCandidateApplicationReviews(this.props.match.params.reviewerCode, seoDecision);
  };
    ...
    return (
      <Dropdown placement="bottomCenter" overlay={
        <Menu>
          ...
          {record.seoDecision === 'PENDING' && <Menu.Item
            onClick={() => {
              this.setState({ selectedCandidate: record, isDecisionModalVisible: true });
            }}>
            Make final decision
        </Menu.Item>}
        </Menu>
...
const mapStateToProps = (state) => ({
  ...
  applicationReviewerSummary: state.candidateApplicationReview.applicationReviewerSummary,
  data: state.candidateApplicationReview.available,
});

const mapDispatchToProps = (dispatch) => ({
  getApplicationReviewerSummary: (reviewerCode) => dispatch(getApplicationReviewerSummary(reviewerCode)),
  getCandidateApplicationReviews: (reviewerCode, seoDecision) => dispatch(getCandidateApplicationReviews(reviewerCode, seoDecision)),
  searchCandidateApplicationReviews: (reviewerCode, seoDecision, searchKey) => dispatch(searchCandidateApplicationReviews(reviewerCode, seoDecision, searchKey)),
});
