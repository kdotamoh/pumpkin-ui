...
const CandidateApplicationReviews = ({
  ...
  const onSeeMoreDetails = async (reviewCode, comment, finalScore) => {
    ...
    try {
      CandidateApplicationService.seeMoreReviewDetails(reviewCode).then(
        (data) => {
          setReviewDetails({
            details: data,
            comment,
  ...
  return pageLoading ? (
    ...
    <React.Fragment>
      ...
      {candidateApplicationSummary.reviewsGroupedByStage?.length > 0
        ? candidateApplicationSummary.reviewsGroupedByStage?.map(
            (reviewsPerStage) => {
              const { reviews } = reviewsPerStage;
              const numberOfNos = reviews.filter((review) =>
                review.decision?.includes('NO')
              ).length;
              const numberOfYes = reviews.filter((review) =>
                review.decision?.includes('YES')
              ).length;
              const numberOfMaybes = reviews.filter((review) =>
                review.decision?.includes('MAYBE')
              ).length;
                      ...
                      <div>
                        <div className="sub-content">
                          ...
                          <div className="flex-1">
                            Decision -{' '}
                            <span style={seoDecisionStyle}>
                              {reviewsPerStage.seoDecision}
                            </span>{' '}
                            by {reviewsPerStage.seoDecisionMadeBy || 'N/A'}
                          </div>
                          <div>
                            {reviewsPerStage.seoRemarks && (
                              <div>
                          ...
                          return (
                            <div style={{ marginBottom: '40px' }}>
                              <div className="data-row">
                                <div className="flex-1">
                                  Reviewer's Name - {review.reviewerName}
                                </div>
                                <div className="flex-1">
                                  SEO YEAR - {review.reviewerSeoYear || 'N/A'}
                                </div>
                                <div className="flex-1 bold">
                                  Total Grade - {review.finalScore || 'N/A'}
                                </div>
                              </div>
                              <div className="data-row">
                                <div className="flex-1">
          ...
          {reviewDetailsLoaded ? (
            <div>
                ...
                {reviewDetails.details?.map((review) => {
                  return (
                    <div className="margin-bottom-15">
                      <div className="flex">
                        <p className="semi-bold" style={{ width: '82%' }}>
                          {review.name}
                        </p>
              ...
              <div>
                ...
                <p className="neg-margin-top-10">{reviewDetails.comment}</p>
              </div>
              <div>
